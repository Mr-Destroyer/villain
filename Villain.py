...
def print_banner():
	...
	print(f"   {''.join(final)}")
	print(f'{END}{padding}           by Mohammad Zim\n')
...
def main():
	...
	if args.update:
		...
		try:
			...
			u = check_output(f'cd {cwd}&&git pull https://github.com/Mr-Destroyer/villain main', shell=True).decode('utf-8')
			...
			if re.search('Updating', u):
				...
				print(f'[{DEBUG}] Consider running "git pull https://github.com/Mr-Destroyer/villain main" inside the project\'s directory.')

		except:
			print(f'[{FAILED}] Update failed. Consider running "git pull https://github.com/Mr-Destroyer/villain main" inside the project\'s directory.')
